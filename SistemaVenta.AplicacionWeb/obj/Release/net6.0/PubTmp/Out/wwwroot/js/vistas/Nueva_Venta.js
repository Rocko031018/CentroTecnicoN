...
function formatoResultados(data) {
    ...
    var contenedor = $(
        `<table width="100%">
           <tr>
             <td style="width:60px">
               <img stile="height:60px;margin-right:10px" src="${data.urlImagen}"/>
             </td>
             <td>
    ...
    fetch("/Venta/RegistrarVenta", {
        ...
        .then(responseJson => {
            if (responseJson.estado) {
                ...
                swal("Completado", `Numero Venta : ${responseJson.objeto.numeroVenta}`, "success")
            }
            else {
                swal("Cancelado", "No se pudo registrar la venta", "error")
            }
        })
})